...
# Association table for Group-Account many-to-many relationship
...
class User(Base):
    ...
    webhook_token = Column(String(255), unique=True, nullable=True, index=True)
...
class Account(Base):
    ...
    role = Column(String(20), nullable=True, index=True)  # master, child, None
    linked_master_id = Column(String(100), nullable=True, index=True)
    copy_status = Column(String(10), default='Off', index=True)  # On, Off
    multiplier = Column(Float, default=1.0)
    last_copied_trade_id = Column(String(100), nullable=True)
    ...
    groups = relationship("Group", secondary=group_accounts, back_populates="accounts")
    
    # Composite unique constraint
    __table_args__ = (
        sa.UniqueConstraint('user_id', 'client_id', name='_user_client_uc'),
    )
...
class Group(Base):
    ...
    accounts = relationship("Account", secondary=group_accounts, back_populates="groups")
    
    # Unique constraint for user + group name
    __table_args__ = (
        sa.UniqueConstraint('user_id', 'name', name='_user_group_name_uc'),
    )
...
class OrderMapping(Base):
    ...
    id = Column(Integer, primary_key=True)
    master_order_id = Column(String(100), nullable=False, index=True)
    child_order_id = Column(String(100), nullable=True)
    master_client_id = Column(String(100), nullable=False, index=True)
    master_broker = Column(String(50), nullable=True)
    child_client_id = Column(String(100), nullable=False, index=True)
    child_broker = Column(String(50), nullable=True)
    symbol = Column(String(50), nullable=False, index=True)
    status = Column(String(20), default='ACTIVE', index=True)  # ACTIVE, COMPLETED, FAILED, CANCELLED, SQUARED_OFF
    timestamp = Column(String(50), nullable=True)
    child_timestamp = Column(String(50), nullable=True)
...
class WebhookLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey('users.id'), nullable=True, index=True)
    webhook_token = Column(String(255), nullable=True, index=True)
...
def setup_logging_with_request_id():
    ...
    if ENVIRONMENT == "production":
        try:
            log_dir = "/tmp/logs"  # Use /tmp for Render
...
def get_user_by_token(token: str):
    ...
    db_session = SessionLocal()
    try:
        return db_session.query(User).filter_by(webhook_token=token).first()
    finally:
        db_session.close()
...
def get_master_accounts():
    ...
    db_session = SessionLocal()
    try:
        masters = db_session.query(Account).filter_by(role='master').limit(MAX_MASTERS_PER_CYCLE).all()
        result = []
        for master in masters:
            children = db_session.query(Account).filter_by(
                role='child', 
...
def get_account_by_client_id(client_id: str):
    ...
    db_session = SessionLocal()
    try:
        return db_session.query(Account).filter_by(client_id=client_id).first()
    finally:
        db_session.close()
...
def load_trades():
    db_session = SessionLocal()
    try:
        return db_session.query(Trade).all()
    finally:
        db_session.close()
...
def load_logs():
    db_session = SessionLocal()
    try:
        webhook_logs = db_session.query(WebhookLog).all()
        system_logs = db_session.query(SystemLog).all()
        return webhook_logs, system_logs
    finally:
        db_session.close()
...
def load_settings():
    ...
    try:
        ...
        for s in db_session.query(Setting).all():
...
def save_account_to_user(owner: str, account: dict):
    ...
    db_session = SessionLocal()
    try:
        user = db_session.query(User).filter_by(email=owner).first()
        if not user:
            webhook_token = ''.join(random.choices(string.ascii_letters + string.digits, k=32))
            user = User(email=owner, webhook_token=webhook_token)
            db_session.add(user)
            db_session.flush()

        # Check if account already exists
        existing_acc = db_session.query(Account).filter_by(
            user_id=user.id, 
        ...
        if existing_acc:
            ...
            for key, value in account.items():
                if hasattr(existing_acc, key):
                    setattr(existing_acc, key, value)
        else:
            # Create new account
            acc = Account(
                user_id=user.id,
...
def get_pending_auth(broker: str) -> dict:
    """Get pending authentication data from database"""
    db_session = SessionLocal()
    try:
        # Clean up expired entries
        db_session.query(PendingAuth).filter(
            PendingAuth.expires_at < datetime.utcnow()
        ).delete()
        db_session.commit()
        
        # Get all pending auth for this broker
        pending_auths = db_session.query(PendingAuth).filter_by(broker=broker).all()
        result = {}
        for auth in pending_auths:
            result[auth.client_id] = auth.auth_data
        return result
    finally:
        db_session.close()

def set_pending_auth(broker: str, client_id: str, auth_data: dict):
    """Set pending authentication data in database"""
    db_session = SessionLocal()
    try:
        # Remove existing entry if any
        db_session.query(PendingAuth).filter_by(
            broker=broker, 
            client_id=client_id
        ).delete()
        
        # Add new entry (expires in 30 minutes)
...
def remove_pending_auth(broker: str, client_id: str):
    """Remove pending authentication data from database"""
    db_session = SessionLocal()
    try:
        db_session.query(PendingAuth).filter_by(
            broker=broker, 
...
def get_user_credentials(identifier: str):
    ...
    try:
        ...
        if account is None:
            account = db_session.query(Account).filter_by(client_id=identifier).first()
        if not account:
            return None
...
def find_account_by_client_id(client_id):
    ...
    db_session = SessionLocal()
    try:
        acc = db_session.query(Account).filter_by(client_id=str(client_id)).first()
        if not acc:
            return None, None

        if acc.role == "child":
            master = db_session.query(Account).filter_by(client_id=acc.linked_master_id).first()
            return acc, master
        return acc, None
    finally:
        db_session.close()
...
def save_log(user_id, symbol, action, quantity, status, response):
    """Persist a trade log entry using database."""
    db_session = SessionLocal()
    try:
        log = TradeLog(
        ...
        for master in masters:
            ...
            for child in children:
                ...
                for order in order_list:
                    ...
                    # Continue with order processing logic...
                    ...
                    # Initialize child broker API and place order
                    try:
...
def get_order_book(client_id):
    ...
    try:
        ...
        master_account = request.db_session.query(Account).filter_by(
...
def zerodha_redirect_handler(client_id):
    ...
    try:
        ...
        try:
            pending = get_pending_auth("zerodha")
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                remove_pending_auth("zerodha", client_id)
...
def master_squareoff():
    ...
    try:
        ...
        active_mappings = request.db_session.query(OrderMapping).join(
        ...
        child_accounts = {a.client_id: a for a in request.db_session.query(Account).filter(
...
def get_master_orders():
    ...
    try:
        ...
        query = request.db_session.query(OrderMapping).join(
...
def init_zerodha_login():
    ...
    set_pending_auth("zerodha", client_id, auth_data)
...
def init_fyers_login():
    ...
    set_pending_auth("fyers", client_id, auth_data)
...
def fyers_redirect_handler(client_id):
    ...
    pending = get_pending_auth("fyers")
    ...
    save_account_to_user(cred.get('owner', username), account)
    remove_pending_auth("fyers", client_id)
    return redirect(url_for('AddAccount'))
...
def signup():
    if request.method == "POST":
        ...
        db_session = SessionLocal()
        try:
            if db_session.query(User).filter_by(email=username).first():
                return render_template("sign-up.html", error="User already exists")
            
            webhook_token = ''.join(random.choices(string.ascii_letters + string.digits, k=32))
            user = User(email=username, webhook_token=webhook_token)
...
def admin_dashboard():
    ...
    db_session = SessionLocal()
    try:
        trades_today = db_session.query(Trade).filter(Trade.timestamp >= start_today, Trade.timestamp < end_today).count()
        active_users = db_session.query(User).filter(User.last_login >= start_today).count()
        failed_trades = db_session.query(Trade).filter_by(status='Failed').count()
    finally:
        db_session.close()
    ...
    for i in range(5):
        ...
        db_session = SessionLocal()
        try:
            trade_counts.append(db_session.query(Trade).filter(Trade.timestamp >= start, Trade.timestamp < end).count())
            signup_counts.append(db_session.query(User).filter(User.subscription_start >= start, User.subscription_start < end).count())
        finally:
            db_session.close()
...
def admin_reset_password(user_id):
    ...
    try:
        ...
        if not user:
            ...
            db_session.commit()
            flash(f'New password for {user.email}: {new_pass}', 'info')
    finally:
        db_session.close()
    return redirect(url_for('admin_users'))
...
def admin_retry_trade(trade_id):
    ...
    try:
        ...
        if not trade:
            ...
            db_session.commit()
            flash('Trade marked for retry', 'success')
    finally:
        db_session.close()
...
def admin_change_subscription(user_id):
    ...
    try:
        ...
        if not user:
            ...
            db_session.commit()
            flash(f'Plan updated to {user.plan} for {user.email}', 'success')
    finally:
        db_session.close()
...
def get_user_accounts():
    ...
    try:
        accounts = request.db_session.query(Account).filter_by(user_id=request.current_user.id).all()
...
def create_account():
    ...
    try:
        ...
        existing = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id,
            client_id=data["client_id"]
        ).first()
        ...
        request.db_session.add(account)
        request.db_session.commit()
...
def update_account(client_id):
    ...
    try:
        ...
        account = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id,
            client_id=client_id
        ).first()
...
@require_user
def delete_account_api(client_id):
    ...
    try:
        ...
        if account.role == "master":
            children = request.db_session.query(Account).filter_by(linked_master_id=client_id).all()
            if children:
                return jsonify({
        ...
        active_mappings = request.db_session.query(OrderMapping).filter(
        ...
        request.db_session.delete(account)
        request.db_session.commit()
...
def get_user_trades():
    ...
    try:
        ...
        query = request.db_session.query(Trade).filter_by(user_id=request.current_user.id)
        ...
        total = query.count()
        trades_query = query.offset(offset).limit(per_page).all()
    ...
    try:
        ...
        try:
            db_stats = {
                ...
                "order_mappings": db_session.query(OrderMapping).count(),
                "active_mappings": db_session.query(OrderMapping).filter_by(status="ACTIVE").count(),
            }
        finally:
            db_session.close()
...
def start_copy():
    ...
    try:
        ...
        child_account = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id, 
            client_id=client_id
        ).first()
        ...
        if master_account.user_id != request.current_user.id:
...
def start_copy_all():
    ...
    try:
        ...
        stopped_children = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id,
            role='child',
            linked_master_id=master_id,
            copy_status='Off'
...
def stop_copy_all():
    ...
    try:
        ...
        active_children = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id,
            role='child',
            linked_master_id=master_id,
            copy_status='On'
...
def square_off():
    ...
    if is_master and parent is None:
        api = broker_api(_account_to_dict(master))
        try:
        ...
        try:
            ...
            for child in children:
                try:
                    api = broker_api(_account_to_dict(child))
                    positions_resp = api.get_positions()
                    positions = positions_resp.get('data', [])
                    match = next((p for p in positions if p.get('tradingSymbol', '').upper() == symbol.upper()), None)
...
def get_order_mappings():
    try:
        mappings = [
            ...
            for m in request.db_session.query(OrderMapping).all()
        ]
...
def child_orders():
    ...
    mappings = request.db_session.query(OrderMapping)
    if master_order_id:
...
def cancel_order():
    try:
        data = request.json
        master_order_id = data.get("master_order_id")
        mappings = request.db_session.query(OrderMapping).filter_by(
        ...
        results = []
        accounts = {a.client_id: a for a in request.db_session.query(Account).all()}
        ...
        for mapping in mappings:
            ...
            try:
                api = broker_api(_account_to_dict(found))
...
def change_master():
    ...
    try:
        ...
        if old_master_id:
            old_master_account = request.db_session.query(Account).filter_by(
                client_id=old_master_id
            ).first()

        new_master_account = request.db_session.query(Account).filter_by(
            user_id=request.current_user.id,
...
def remove_child():
    ...
    try:
        ...
        active_mappings = request.db_session.query(OrderMapping).filter_by(
...
def remove_master():
    ...
    try:
        ...
        linked_children = request.db_session.query(Account).filter_by(
            role='child',
...
def get_groups():
    ...
    groups = request.db_session.query(Group).filter_by(user_id=request.current_user.id).all()
...
def create_group():
    ...
    if request.db_session.query(Group).filter_by(user_id=request.current_user.id, name=name).first():
        return jsonify({"error": "Group already exists"}), 400

    group = Group(name=name, user_id=request.current_user.id)
    for cid in members:
        acc = request.db_session.query(Account).filter_by(user_id=request.current_user.id, client_id=cid).first()
        if acc:
...
def add_account_to_group(group_name):
    ...
    group = request.db_session.query(Group).filter_by(user_id=request.current_user.id, name=group_name).first()
    if not group:
        return jsonify({"error": "Group not found"}), 404
    acc = request.db_session.query(Account).filter_by(user_id=request.current_user.id, client_id=client_id).first()
    if not acc:
        return jsonify({"error": "Account not found"}), 404
...
def place_group_order():
    ...
    accounts = [_account_to_dict(acc) for acc in group.accounts]
    results = []
    for acc in accounts:
        try:
            api = broker_api(acc)
            ...
            record_trade(request.current_user.email, symbol, action.upper(), quantity, order_params.get('price'), status)
        except Exception as e:
...
def get_alerts():
    ...
    try:
        logs = (
            db_session.query(TradeLog).filter_by(user_id=user_id, status="ALERT")
...
def register_user():
    ...
    db_session = SessionLocal()
    try:
        user = db_session.query(User).filter_by(webhook_token=token).first()
        if not user:
            user = User(webhook_token=token, email=f"webhook_{token}@generated.com")
            user.set_password('generated_password')
            db_session.add(user)
            db_session.flush()

        account = db_session.query(Account).filter_by(user_id=user.id, client_id=client_id).first()
...
def get_logs():
    ...
    try:
        rows = (
            db_session.query(TradeLog).filter_by(user_id=user_id)
            .order_by(TradeLog.id.desc())
            .limit(100)
            .all()
        )
...
def get_portfolio(user_id):
    ...
    db_session = SessionLocal()
    try:
        user_rec = db_session.query(User).filter_by(webhook_token=user_id).first()
        if not user_rec:
            user_rec = db_session.query(User).filter_by(email=user_id).first()
        if not user_rec:
...
def user_profile():
    ...
    if request.method == "POST":
        ...
        if action == "save_profile":
            ...
            request.current_user.name = f"{first_name} {last_name}".strip()
            ...
            if file and file.filename:
                ...
                request.current_user.profile_image = os.path.join("profile_images", filename)
    ...
    profile_data = {
        "email": username,
        "first_name": (request.current_user.name or "").split(" ")[0] if request.current_user.name else "",
        "last_name": (request.current_user.name or "").split(" ")[1] if request.current_user.name and len(request.current_user.name.split(" ")) > 1 else "",
        "plan": request.current_user.plan,
    }
...
def cleanup_expired_auth():
    """Clean up expired pending authentication data"""
    try:
        db_session = SessionLocal()
        try:
            expired_count = db_session.query(PendingAuth).filter(
                PendingAuth.expires_at < datetime.utcnow()
            ).delete()
            if expired_count > 0:
                db_session.commit()
                logger.info(f"Cleaned up {expired_count} expired pending auth records")
        finally:
            db_session.close()
    except Exception as e:
        logger.error(f"Failed to cleanup expired auth: {str(e)}")
